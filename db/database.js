...
const getallCourses = (callback) => {
  let sql = `select * from CoursesLast ;`;
  connection.query(sql, (err, data) => {
    if (err) throw callback(err, null);
    callback(null, data);
  });
...
const getoneCours = (arr, callback) => {
  let sql = "select * FROM CoursesLast WHERE id = ?";
  connection.query(sql, arr, (err, data) => {
    if (err) throw callback(err, null);
    callback(null, data);
  });
...
const addCours = (arr, callback) => {
  let sql =
    "insert into CoursesLast (nameCours,imageUrl ,description ,Creator )  values (?,?,?,?);";
  connection.query(sql, arr, (err, data) => {
    if (err) throw callback(err, null);
    callback(null, data);
  });
...
const addModule = (arr, callback) => {
  let sql =
    "insert into ModuleLast (title ,videoUrl,coursId) values (?,?,?)";
  connection.query(sql, arr, (err, data) => {
    if (err) throw callback(err, null);
    callback(null, data);
  });
...
const getModules = (arr, callback) => {
  let sql =
    "select * from ModuleLast where coursId = ? ORDER BY id ASC; ";
  connection.query(sql, arr, (err, data) => {
    if (err) throw callback(err, null);
    callback(null, data);
  });
};
const getPragraphs = (arr, callback) => {
  let sql =
    "select * from PragraphLast where ModuleId = ?; ";
  connection.query(sql, arr, (err, data) => {
    if (err) throw callback(err, null);
    callback(null, data);
  });
};
const addParagraphs = (arr, callback) => {
  let sql =
    "insert into PragraphLast (title ,imageUrl,content,ModuleId) values (?,?,?,?)";
  connection.query(sql, arr, (err, data) => {
    if (err) throw callback(err, null);
    callback(null, data);
  });
...
module.exports = {
  ...
  assignCohort,
  addCours,
  getallusers,addParagraphs,
  logusers,addModule,getModules,getPragraphs
};
